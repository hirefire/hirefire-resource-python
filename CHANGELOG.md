...
- The dispatcher is now fork-aware: a forked worker (e.g. Gunicorn with `--preload`) detects that the inherited dispatcher thread did not survive the fork and starts a fresh one on the first request, instead of silently never dispatching. Locks that the fork copied in a locked state are re-created in the child (via `os.register_at_fork`), so a worker cannot deadlock on a mutex the parent happened to hold at the instant of the fork.
